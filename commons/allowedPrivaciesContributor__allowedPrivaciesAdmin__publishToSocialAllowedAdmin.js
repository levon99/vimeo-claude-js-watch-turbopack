{
  ...
  _v0.s(["getTranslations", 0, () => (null === _v4 && (_v4 = Object.freeze({
    ...
    VideoFileLinkDescription: (0, _v3.translate)({
      singular: "Allow sharing direct video URLs as non-expiring external links",
      dictionary: {
        es: {
          singular: "Permitir compartir URLs directas de vídeo como enlaces externos sin caducidad"
        },
        "de-DE": {
          singular: "Teilen direkter Video-URLs als nicht ablaufende externe Links erlauben"
        },
        "fr-FR": {
          singular: "Autoriser le partage d'URL vidéo directes sous forme de liens externes sans expiration"
        },
        "ja-JP": {
          singular: "直接のビデオURLを、有効期限のない外部リンクとして共有できるようにする"
        },
        "ko-KR": {
          singular: "직접 동영상 URL을 만료되지 않는 외부 링크로 공유 허용"
        },
        "pt-BR": {
          singular: "Permitir o compartilhamento de URLs diretas de vídeo como links externos sem expiração"
        },
        "zh-CN": {
          singular: "允许将直接视频 URL 作为不会过期的外部链接进行共享"
        }
      }
    }),
    VideoFileLinkTooltip: (0, _v3.translate)({
      singular: "When enabled, team members can access a dedicated modal to copy direct, non-expiring streaming and download URLs for external sharing. This does not affect the Download button or other sharing options.",
      dictionary: {
        es: {
          singular: "Cuando está habilitado, los miembros del equipo pueden acceder a una ventana modal dedicada para copiar URLs directas de streaming y de descarga que no caducan, para compartirlas externamente. Esto no afecta al botón Descargar ni a otras opciones para compartir."
        },
        "de-DE": {
          singular: "Wenn aktiviert, können Teammitglieder ein spezielles Modal öffnen, um direkte, nicht ablaufende Streaming- und Download-URLs zum externen Teilen zu kopieren. Dies hat keinen Einfluss auf den Download-Button oder andere Freigabeoptionen."
        },
        "fr-FR": {
          singular: "Lorsque cette option est activée, les membres de l'équipe peuvent accéder à une fenêtre modale dédiée pour copier des URL directes de streaming et de téléchargement sans expiration en vue d'un partage externe. Cela n'affecte pas le bouton Télécharger ni les autres options de partage."
        },
        "ja-JP": {
          singular: "有効にすると、チームメンバーは専用のモーダルを開いて、外部共有用の直接で有効期限のないストリーミングおよびダウンロード用URLをコピーできます。これは「ダウンロード」ボタンやその他の共有オプションには影響しません。"
        },
        "ko-KR": {
          singular: "활성화하면 팀원들은 외부 공유를 위해 직접 사용 가능한 만료되지 않는 스트리밍 및 다운로드 URL을 복사할 수 있는 전용 모달에 접근할 수 있습니다. 이는 Download 버튼이나 다른 공유 옵션에는 영향을 주지 않습니다."
        },
        "pt-BR": {
          singular: "Quando ativado, os membros da equipe podem acessar um modal dedicado para copiar URLs diretas de streaming e de download que não expiram para compartilhamento externo. Isso não afeta o botão Download nem outras opções de compartilhamento."
        },
        "zh-CN": {
          singular: "启用后，团队成员可以打开专用模态窗口，复制用于外部共享的直接、不会过期的流媒体和下载 URL。此设置不会影响“下载”按钮或其他共享选项。"
        }
      }
    }),
