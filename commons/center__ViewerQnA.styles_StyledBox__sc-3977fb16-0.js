{
  "use strict";

  var _v1 = _v0.i(0),
    _v2 = _v0.i(0),
    _v3 = _v0.i(0),
    _v4 = _v0.i(0),
    _v5 = _v0.i(0),
    _v6 = _v0.i(0),
    _v7 = _v0.i(0),
    _v8 = _v0.i(0),
    _v9 = _v0.i(0),
    _v10 = _v0.i(0);
  _v0.i(0);
  var _v11 = _v0.i(0);
  let _v12 = ({
    ..._v0
  }) => (0, _v1.jsx)(_v2.Flex, {
    w: "100%",
    h: "100%",
    alignItems: "center",
    justifyContent: "center",
    ..._v0
  });
  var _v13 = (0, _v6.default)(_v8.Box).withConfig({
    displayName: "ViewerQnA.styles___StyledBox",
    componentId: "sc-3977fb16-0"
  })`${_v0 => _v0.$_css}`;
  (0, _v6.default)(_v8.Box).withConfig({
    displayName: "ViewerQnA.styles___StyledBox2",
    componentId: "sc-3977fb16-1"
  })`${_v11.VIEWER_QNA_ANSWER_PLACEHOLDER_ANIMATION}`;
  var _v14 = (0, _v6.default)(_v9.Input).withConfig({
    displayName: "ViewerQnA.styles___StyledInput",
    componentId: "sc-3977fb16-2"
  })`
        &:disabled {
          pointer-events: none;
          opacity: 60%;
        }
      `;
  _v0.s(["LoaderWrapper", 0, _v12, "QuestionInput", 0, ({
    isPlayer: _v0 = !1,
    isLoading: _v1,
    ..._v2
  }) => {
    let _v3 = (0, _v7.useRef)(null);
    return (0, _v7.useEffect)(() => {
      _v0 && window.addEventListener("message", _v0 => {
        "componentVisibilityChange" === _v0.data.event && _v0.data.data && _v3.current?.focus();
      });
    }, []), (0, _v1.jsx)(_v14, {
      alignSelf: "flex-end",
      ref: _v3,
      sx: {
        paddingRight: "40px",
        ...(_v1 ? {
          overflow: "hidden",
          textOverflow: "ellipsis"
        } : {})
      },
      ..._v2
    });
  }, "ResponsiveWrapper", 0, ({
    aiThinking: _v0,
    isPlayer: _v1 = !1,
    ..._v2
  }) => (0, _v1.jsx)(_v13, {
    w: "100%",
    h: "100%",
    flex: "1 0 auto",
    maxH: "100%",
    minH: "0",
    borderRadius: "1rem",
    ..._v2,
    $_css: _v0 ? "" : _v1 ? `
      background-color: transparent;
    ` : ""
  }), "ViewerQnAHeaderWrapper", 0, ({
    ..._v0
  }) => (0, _v1.jsx)(_v10.HStack, {
    borderBottom: "0.5pt solid",
    borderBottomColor: "stroke",
    maxH: "80px",
    ..._v0
  })], 0), _v0.s(["AiActivationPanel", 0, ({
    isUiRefresh: _v0 = !1
  }) => _v0 ? (0, _v1.jsxs)(_v2.Flex, {
    w: "100%",
    h: "100%",
    flexDirection: "column",
    alignItems: "center",
    justifyContent: "center",
    px: "12px",
    gap: "12px",
    "data-testid": "ai-activation-panel",
    children: [(0, _v1.jsx)(_v3.Spinner, {
      size: "md",
      color: "text-secondary"
    }), (0, _v1.jsxs)(_v2.Flex, {
      flexDirection: "column",
      alignItems: "center",
      width: "100%",
      children: [(0, _v1.jsx)(_v4.Text, {
        color: "text-primary",
        fontFamily: "heading",
        fontSize: "heading-md",
        lineHeight: 1.4,
        textAlign: "center",
        children: _v5.askVimeoAi
      }), (0, _v1.jsx)(_v4.Text, {
        color: "text-secondary",
        fontSize: "body-lg",
        lineHeight: 1.4,
        textAlign: "center",
        marginTop: "0px",
        children: _v5.aiIsAnalyzing
      })]
    })]
  }) : (0, _v1.jsxs)(_v12, {
    flexDirection: "column",
    p: "16px",
    children: [(0, _v1.jsx)(_v3.Spinner, {
      size: "xl"
    }), (0, _v1.jsx)(_v4.Text, {
      marginTop: "16px",
      fontSize: "16px",
      textAlign: "center",
      color: "text-primary",
      variant: "body-xl",
      children: _v5.aiIsAnalyzing
    })]
  })], 0);
  var _v15 = _v0.i(0),
    _v16 = _v0.i(0),
    _v17 = _v0.i(0),
    _v18 = _v11;
  let _v19 = ({
      maxWidth: _v0,
      bgOffset: _v1 = 0,
      gradient: _v2 = _v18.SHIMMER_GRADIENT
    }) => (0, _v1.jsx)(_v17.motion.div, {
      style: {
        width: _v0,
        height: "200%",
        opacity: "100%",
        borderRadius: "inherit",
        position: "relative",
        bottom: "50%",
        filter: "blur(16px)"
      },
      initial: {
        left: -(1.5 * _v0 - _v1)
      },
      animate: {
        left: 1.5 * _v0 + _v1,
        opacity: [0, .3, .7, .5, .1, 0],
        transition: {
          duration: 2,
          repeatType: "loop",
          ease: "linear",
          repeat: 1 / 0
        }
      },
      children: (0, _v1.jsx)(_v8.Box, {
        w: "200%",
        h: "200%",
        bg: _v2
      })
    }),
    _v20 = ({
      isNeutral: _v0,
      ..._v1
    }) => (0, _v1.jsx)(_v8.Box, {
      borderRadius: _v0 ? "12px" : "8px",
      bg: "background-blur",
      overflow: "hidden",
      ..._v1,
      children: _v1.maxWidth && _v1.bgOffset && (0, _v1.jsx)(_v19, {
        maxWidth: _v1.maxWidth,
        bgOffset: _v1.bgOffset,
        gradient: _v0 ? _v18.NEUTRAL_SHIMMER_GRADIENT : _v18.SHIMMER_GRADIENT
      })
    });
  _v0.s(["AskQuestionPanel", 0, ({
    isUiRefresh: _v0 = !1
  }) => {
    let [_v1, _v2] = (0, _v7.useState)(),
      _v3 = (0, _v7.useRef)(null),
      {
        question: _v4
      } = (0, _v16.useQuestionContext)();
    return ((0, _v7.useLayoutEffect)(() => {
      _v2(_v3.current?.offsetWidth);
    }, []), _v0) ? (0, _v1.jsxs)(_v2.Flex, {
      ref: _v3,
      w: "100%",
      h: "100%",
      p: "16px",
      flexDirection: "column",
      "data-testid": "ask-question-panel",
      children: [_v4 ? (0, _v1.jsx)(_v2.Flex, {
        justifyContent: "flex-end",
        width: "100%",
        mb: "24px",
        children: (0, _v1.jsx)(_v8.Box, {
          bg: "button-secondary-default",
          borderRadius: "12px 0px 12px 12px",
          px: "12px",
          py: "8px",
          maxWidth: "320px",
          width: "100%",
          children: (0, _v1.jsx)(_v8.Box, {
            color: "text-primary",
            fontSize: "body-md",
            lineHeight: 1.4,
            children: _v4
          })
        })
      }) : null, (0, _v1.jsxs)(_v15.VStack, {
        gap: "8px",
        w: "100%",
        alignItems: "flex-start",
        children: [(0, _v1.jsx)(_v20, {
          h: "24px",
          w: "100%",
          maxWidth: _v1,
          bgOffset: 15,
          isNeutral: !0
        }), (0, _v1.jsx)(_v20, {
          h: "24px",
          w: "70%",
          maxWidth: _v1,
          bgOffset: 30,
          isNeutral: !0
        }), (0, _v1.jsx)(_v20, {
          h: "24px",
          w: "85%",
          maxWidth: _v1,
          bgOffset: 30,
          isNeutral: !0
        })]
      })]
    }) : (0, _v1.jsx)(_v2.Flex, {
      ref: _v3,
      w: "100%",
      h: "100%",
      p: "16px",
      flexDirection: "column",
      justifyContent: "space-between",
      "data-testid": "ask-question-panel",
      children: (0, _v1.jsxs)(_v15.VStack, {
        gap: "8px",
        w: "100%",
        alignItems: "flex-start",
        children: [(0, _v1.jsx)(_v20, {
          h: "16px",
          w: "70%",
          maxWidth: _v1,
          bgOffset: 15
        }), (0, _v1.jsx)(_v20, {
          h: "16px",
          w: "100%",
          maxWidth: _v1,
          bgOffset: 30
        }), (0, _v1.jsx)(_v20, {
          h: "16px",
          w: "70%",
          maxWidth: _v1,
          bgOffset: 45
        }), (0, _v1.jsx)(_v20, {
          h: "16px",
          w: "60%",
          maxWidth: _v1,
          bgOffset: 60
        })]
      })
    });
  }], 0);
}